...
namespace NativeCollections
{
    ...
    public unsafe struct UnsafeXoshiro128 : IIsCreated, IEquatable<UnsafeXoshiro128>
    {
        /// <summary>
        ///     State0
        /// </summary>
        ...
        public UnsafeXoshiro128(ReadOnlySpan<byte> buffer)
        {
            ...
            var random = Unsafe.ReadUnaligned<UnsafeXoshiro128>(ref MemoryMarshal.GetReference(buffer));
            if (!random.IsCreated)
        ...
        {
            var data = MemoryMarshal.CreateSpan(ref Unsafe.As<uint, byte>(ref _s0), Unsafe.SizeOf<UnsafeXoshiro128>());
            do
            {
        ...
        public void NextBytes(Span<byte> buffer)
        {
            ...
            if (!buffer.IsEmpty)
            {
                ...
                Unsafe.CopyBlockUnaligned(ref MemoryMarshal.GetReference(buffer), ref Unsafe.As<uint, byte>(ref num8), (uint)buffer.Length);
                var num9 = s1 << 9;
                var num10 = num1 ^ s0;
                var num11 = num2 ^ s1;
                s1 ^= num10;
        ...
        ///     Empty
        /// </summary>
        public static UnsafeXoshiro128 Empty => new();
    }
}