...
namespace NativeCollections
{
    ...
    public unsafe struct UnsafeUInt64MemoryPool : IDisposable
    {
        ...
        public UnsafeUInt64MemoryPool(int length, int maxFreeSlabs, int alignment)
        {
            ...
            var buffer = NativeMemoryAllocator.AlignedAlloc((uint)(alignedSlabByteCount + 64 * nodeByteCount), (uint)alignment);
        ...
        public void* Rent()
        {
            ...
            if (slab->Bitmap == ulong.MaxValue)
            {
                ...
                if (slab->Bitmap == ulong.MaxValue)
                {
                    if (_freeSlabs == 0)
                    {
                        var buffer = NativeMemoryAllocator.AlignedAlloc((uint)(alignedSlabByteCount + 64 * nodeByteCount), (uint)_alignment);
