...
namespace NativeCollections
{
    ...
    /// </remarks>
    internal static unsafe class Tlsf64
    {
        ...
        public static void tlsf_remove_pool(void* tlsf, void* pool)
        {
            ...
            var block = offset_to_block(pool, unchecked((ulong)-(long)block_header_overhead));
            int fl = 0, sl = 0;
            mapping_insert(block_size(block), &fl, &sl);
            remove_free_block(control, block, fl, sl);
        }
        ...
        public static void* tlsf_realloc(void* tlsf, void* ptr, ulong size)
        {
            var control = (control_t*)tlsf;
            void* p = null;
            if (!UnsafeHelpers.IsNull(ptr) && size == 0)
                tlsf_free(tlsf, ptr);
            else if (UnsafeHelpers.IsNull(ptr))
