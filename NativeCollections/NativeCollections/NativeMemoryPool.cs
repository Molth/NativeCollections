...
namespace Native.Collections
{
    ...
    public readonly unsafe struct NativeMemoryPool : IDisposable
    {
        ...
        public NativeMemoryPool(int size, int length, int maxFreeSlabs)
        {
            ...
            var slab = (NativeMemorySlab*)NativeMemoryAllocator.Alloc(sizeof(NativeMemorySlab));
            slab->Next = null;
            slab->Previous = null;
            var nodeSize = sizeof(NativeMemoryNode) + length;
            var array = (byte*)NativeMemoryAllocator.Alloc(size * nodeSize);
            slab->Array = array;
        ...
        public void Dispose()
        {
            ...
            while (node != null)
            {
                NativeMemoryAllocator.Free(node->Array);
                var temp = node;
                node = node->Next;
        ...
        public byte* Rent()
        {
            ...
            if (slab->Count == 0)
            {
                ...
                {
                    slab = (NativeMemorySlab*)NativeMemoryAllocator.Alloc(sizeof(NativeMemorySlab));
        ...
        public void Return(byte* ptr)
        {
            ...
            if (slab->Count == _handle->Size && slab != _handle->Slab)
            {
                ...
                {
                    NativeMemoryAllocator.Free(slab->Array);
