...
namespace NativeCollections
{
    ...
    [BindingType(typeof(NativeMemory))]
#else
    [BindingType(typeof(Marshal))]
#endif
    public static unsafe class NativeMemoryAllocator
    {
        ...
        {
            if (UnsafeHelpers.IsNull(left) && UnsafeHelpers.IsNull(right))
                return true;
            if (UnsafeHelpers.IsNull(left) || UnsafeHelpers.IsNull(right))
                ThrowHelpers.ThrowArgumentNullException(UnsafeHelpers.IsNull(left) ? ExceptionArgument.left : ExceptionArgument.right);
        ...
        /// <param name="reference">The object whose address is obtained.</param>
