...
namespace Examples
{
    ...
    /// </remarks>
    public sealed class RingBufferNotPow2<T>
    {
        /// <summary>
        ...
        public RingBufferNotPow2(int capacity)
        {
            ...
            _fastModMultiplier = Environment.Is64BitProcess ? HashHelpers.GetFastModMultiplier((uint)capacity) : 0;
        ...
        public int Count
        {
            get
            {
                ...
                {
                    head = (int)GetSlotIndex((uint)head);
                    tail = (int)GetSlotIndex((uint)tail);
        ...
        public bool TryDequeue(out T? result)
        {
            ...
            while (true)
            {
                ...
                var slotsIndex = GetSlotIndex((uint)currentHead);
                var sequenceNumber = Volatile.Read(ref Unsafe.Add(ref slot, slotsIndex).SequenceNumber);
                ...
                if (diff == 0)
                {
                    ...
                    {
                        result = Unsafe.Add(ref slot, slotsIndex).Item;
                        if (RuntimeHelpers.IsReferenceOrContainsReferences<T>())
                            Unsafe.Add(ref slot, slotsIndex).Item = default;
                        Volatile.Write(ref Unsafe.Add(ref slot, slotsIndex).SequenceNumber, currentHead + _slotsLength);
                        return true;
                    }
                }
                else if (diff < 0)
        ...
        public bool TryEnqueue(T? item)
        {
            ...
            {
                var currentTail = Volatile.Read(ref _headAndTail.Tail);
                var slotsIndex = GetSlotIndex((uint)currentTail);
                var sequenceNumber = Volatile.Read(ref Unsafe.Add(ref slot, slotsIndex).SequenceNumber);
                ...
                if (diff == 0)
                {
                    ...
                    {
                        Unsafe.Add(ref slot, slotsIndex).Item = item;
                        Volatile.Write(ref Unsafe.Add(ref slot, slotsIndex).SequenceNumber, currentTail + 1);
                        return true;
                    }
                }
                else if (diff < 0)
