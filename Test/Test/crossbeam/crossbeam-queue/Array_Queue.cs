...
namespace crossbeam
{
    ...
    internal static unsafe class Array_Queue
    {
        ...
        public struct ArrayQueue<T> : IIsCreated
        {
            ...
            private readonly Slot<T>[] buffer;
            ...
            public ArrayQueue(nuint cap)
            {
                ...
                one_lap = (nuint)BitOperations.RoundUpToPowerOf2((uint)(cap + 1));
            }

            private Result<T> push_or_else(T? value, delegate* managed<ref ArrayQueue<T>, T?, nuint, nuint, ref Slot<T>, Result<T>> f)
            {
                ...
                while (true)
                {
                    ...
                    ref var slot = ref this.buffer.get_unchecked(index);
                    ...
                    else if (stamp.wrapping_add(this.one_lap) == tail + 1)
                    {
                        ...
                        var result = f(ref this, value, tail, new_tail, ref slot);
                        if (result.is_err())
                            return result;
            ...
            public Result<T> push(T? value)
            {
                ...
                static Result<T> f(ref ArrayQueue<T> self, T? v, nuint tail, nuint _, ref Slot<T> __)
                {
            ...
            public Option<T> force_push(T? value)
            {
                ...
                static Result<T> f(ref ArrayQueue<T> self, T? v, nuint tail, nuint new_tail, ref Slot<T> slot)
                {
